...
async function getGuestsByEventId(event_id) {
    const res = await fetch(`${process.env.BACKEND_API}/api/v1/invitation/guests/${event_id}`)
...
export default async function RSVPTrackingPage({ params }: { params: { id: string } }) {
    ...
    const guests = process.env.NODE_ENV === 'development' ? mockEventDetails.guestList :  await getGuestsByEventId(params.id);
